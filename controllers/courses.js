...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.id != bootcamp.user && req.user.role != 'admin') {
    return next(
      new ErrorResponse(
        `you cannot add course to this bootcamp, because you are not the owner !`,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.id != course.user && req.user.role != 'admin') {
    return next(
      new ErrorResponse(
        `you cannot update course to this bootcamp, because you are not the owner !`,
        401
      )
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.id != course.user && req.user.role != 'admin') {
    return next(
      new ErrorResponse(
        `you cannot remove course to this bootcamp, because you are not the owner !`,
        401
      )
    );
  }
